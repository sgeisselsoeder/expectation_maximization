...
class GMM:
    ...
    def run(self):
        ...
        for m, c in zip(self.mu, self.cov):
            ...
            ax0.contour(np.sort(self.X[:, 0]), np.sort(self.X[:, 1]), multi_normal.pdf(
                self.XY).reshape(len(self.X), len(self.X)), colors='black', alpha=0.3)
        ...
        for i in range(self.iterations):
            ...
            self.cov = []
            self.pi = []
            log_likelihood = []  # TODO: check this defect!
            ...
            for c in range(len(r_ic[0])):
                ...
                self.cov.append(
                    ((1/m_c)*np.dot((np.array(r_ic[:, c]).reshape(len(self.X), 1)*(self.X-mu_c)).T, (self.X-mu_c)))+self.reg_cov)
    ...
    def predict(self, Y):
        ...
        for m, c in zip(self.mu, self.cov):
            multi_normal = multivariate_normal(mean=m, cov=c)
            ax2.contour(np.sort(self.X[:, 0]), np.sort(self.X[:, 1]), multi_normal.pdf(
                self.XY).reshape(len(self.X), len(self.X)), colors='black', alpha=0.3)
